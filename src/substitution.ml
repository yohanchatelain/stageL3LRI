...
let rec pattern sub t = function
  | Pvar s ->
    Env.add s t sub
  | Pconstr (s,[p]) -> pattern sub (destruct s t) p
  | Pconstr (s, pl) ->
    let i = ref (-1) in
...
let rec expr sub = function
  ...
  | Ematch (e,bl) ->
    ...
    sum (List.rev t),c
  | Econstr (s,el) ->
    let (l,cl) = List.fold_left 
      (fun (t,cl) e ->
	let (t',cl') = expr sub e in
	t'::t,List.rev_append cl cl'
      ) ([],[]) el 
    in
...
let compose sub1 sub2 =
  Env.map 
    (fun t ->
      let acc = ref t in
      Env.iter (fun k' t' -> acc := subst k' t' !acc) sub1;
      Collapsing.collapse (!acc) 
    ) sub2
